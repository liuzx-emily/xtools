...
zxTreetable.prototype = {
    ...
    build_html: function() {
        var add_row = function(data, index, parentId, level) {
            ...
            table_html += `
                    <tr  class="${levelRelatedClass}" data_id="${data.id}" data_parentid="${parentId}">
            ...
            $.each(_this.title, function(_index, each_title) {
                if (each_title.crud) {
                    ...
                    if (_index === 0) {
                        ...
                        table_html += `<td title="${td_data}" class="controltd" style="padding-left:${24*level}px">${icon_html}${td_data}</td>`;
                    } else {
                        // 普通td
            ...
            if (data.child && data.child.length > 0) {
                data.child.forEach(function(item, index) {
                    add_row(item, index, data.id, level + 1);
                });
            }
        };
        var _this = this;
